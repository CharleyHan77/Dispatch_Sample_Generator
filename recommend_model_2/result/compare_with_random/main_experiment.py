...
- `pareto_recommended_strategy_results_{timestamp}.json` - 帕累托推荐策略测试结果
- `pareto_frontier_analysis_{timestamp}.png` - 帕累托前沿分析图
...
def main():
    ...
    # 创建实验结果目录（使用时间戳子目录，绝对路径）
    exp_result_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "exp_result", f"exp_{timestamp}"))
    ...
    RECOMMENDER_PATH = os.path.join(BASE_DIR, 'recommend_model_2/initialization_strategy_recommender.py')
    RANDOM_SCRIPT = os.path.join(BASE_DIR, 'recommend_model_2/result/compare_with_random/random_initialization_test.py')
    RECOMMENDED_SCRIPT = os.path.join(BASE_DIR, 'recommend_model_2/result/compare_with_random/pareto_recommended_strategy_test.py')
    COMPARISON_SCRIPT = os.path.join(BASE_DIR, 'recommend_model_2/result/compare_with_random/performance_comparison.py')
    ...
    # 步骤2: 帕累托推荐策略性能测试
    logger.info("步骤2: 帕累托推荐策略性能测试")
    recommended_script = RECOMMENDED_SCRIPT
    
    # 直接通过参数调用子脚本，传递时间戳
    if not run_python_script(recommended_script, "帕累托推荐策略性能测试", logger, os.path.abspath(fjs_file_path), timestamp):
        logger.error("帕累托推荐策略测试失败，实验终止")
        return
    
    # 检查推荐策略结果（重试机制，绝对路径）
    for _ in range(5):
        recommended_result_files = list(Path(exp_result_dir).glob("pareto_recommended_strategy_results_*.json"))
        if recommended_result_files:
            break
        time.sleep(1)
    if not recommended_result_files:
        logger.error("未找到帕累托推荐策略测试结果，实验终止")
        return
