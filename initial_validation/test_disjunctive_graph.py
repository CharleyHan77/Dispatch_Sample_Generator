...
def create_disjunctive_graph(parameters: Dict[str, Any], instance_name: str, save_path: Optional[str] = None, title: str = "析取图") -> nx.DiGraph:
    """
    ...
    for machine, edges in machine_edges.items():
        # 设置基础弧度和变化范围
        base_rad = 0.2  # 使用较小的基础弧度
        # 根据机器编号调整弧度，使不同机器的弧线更容易区分
        if machine % 2 == 0:
            rad = base_rad + ((machine - 1) % 3) * 0.1  # 正弧度，在0.2到0.4之间变化
        else:
            rad = -(base_rad + ((machine - 1) % 3) * 0.1)  # 负弧度，在-0.2到-0.4之间变化
        ...
        nx.draw_networkx_edges(G, node_positions,
                             ...
                             edge_color='gray',
                             style='dashed',
                             arrows=True,  # 启用FancyArrowPatches
                             arrowsize=1,  # 设置很小的箭头大小，使其几乎不可见
                             width=0.8,
                             alpha=0.5,  # 降低透明度
...
def main():
    ...
    try:
        ...
        # 获取数据集根目录
        dataset_root = os.path.join(project_root, "dataset\Hurink")
        ...
        for dataset_name in os.listdir(dataset_root):
            ...
            # 创建对应的输出目录
            output_dir = os.path.join(project_root, "output", "disjunctive_graphs_new\Hurink", dataset_name)
