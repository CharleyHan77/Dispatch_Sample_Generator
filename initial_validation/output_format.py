"""
用于基于Brandimarte数据集的初始化方法验证，并得到的输出规范化json文件（验证统计结果 + 10维基础与 加工时间特征 + 三种初始化方案）
"""
...
def plot_convergence_curves(convergence_data: dict, save_path: str = None, dataset_name: str = "Brandimarte", instance_name: str = "Mk01.fjs"):
    """
    绘制收敛曲线
    ...
    for init_method, curves in convergence_data.items():
        ...
        # 添加坐标值标注
        plt.annotate(
            f'({convergence_idx}, {convergence_value:.2f})',
    ...
    plt.xlabel('迭代数')
    plt.ylabel('Makespan')
    plt.title(f'{dataset_name}/{instance_name} 三种初始化方式的平均收敛曲线（当前元启发算法最大迭代数100 每种初始化方式运行20次）')
    plt.legend()
    plt.grid(True)
...
def generate_output_json(parameters, results, convergence_times, dataset_name="Brandimarte", instance_name="Mk01.fjs", max_iterations=100, runs=20):
    """
    生成包含所有所需信息的JSON输出
    """
    # 提取特征
    ...
    output_data = {
        ...
        "initialization_methods": {
            "heuristic": {
                "mean": float(np.mean(results["heuristic"])),
                "std": float(np.std(results["heuristic"])),
                "min": float(np.min(results["heuristic"])),
                "max": float(np.max(results["heuristic"])),
                "avg_convergence_generation": float(np.mean(convergence_times["heuristic"])),
                "convergence_generation_std": float(np.std(convergence_times["heuristic"]))
            },
            "mixed": {
                "mean": float(np.mean(results["mixed"])),
                "std": float(np.std(results["mixed"])),
                "min": float(np.min(results["mixed"])),
                "max": float(np.max(results["mixed"])),
                "avg_convergence_generation": float(np.mean(convergence_times["mixed"])),
                "convergence_generation_std": float(np.std(convergence_times["mixed"]))
            },
            "random": {
                "mean": float(np.mean(results["random"])),
                "std": float(np.std(results["random"])),
                "min": float(np.min(results["random"])),
                "max": float(np.max(results["random"])),
                "avg_convergence_generation": float(np.mean(convergence_times["random"])),
                "convergence_generation_std": float(np.std(convergence_times["random"]))
            }
        }
    }
...
if __name__ == '__main__':
    # 获取数据集文件的绝对路径
    dataset_name = "Brandimarte"
    instance_name = "Mk01.fjs"
    file_path = os.path.join(project_root, "dataset", dataset_name, instance_name)
    
    # 设置执行次数和最大迭代次数
    runs = 20
    max_iterations = config.maxGen
    ...
    for init_method in results.keys():
        ...
        for i in range(runs):
            ...
            print(f"一次执行的早期收敛代数：{convergence_idx}")
            print("="*30)
    ...
    # 生成JSON输出
    output_data = generate_output_json(parameters, results, convergence_times, dataset_name, instance_name)
    
    # 创建输出目录
    output_dir = os.path.join(project_root, "output")
    record_dir = os.path.join(output_dir, "record_result")
    convergence_dir = os.path.join(output_dir, "convergence_curves")
    os.makedirs(record_dir, exist_ok=True)
    os.makedirs(convergence_dir, exist_ok=True)
    
    # 保存JSON文件
    json_filename = f"{dataset_name}_{instance_name}_validation_results.json"
    output_file = os.path.join(record_dir, json_filename)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)
    
    print(f"\nJSON输出已保存到: {output_file}")
    
    # 绘制收敛曲线
    convergence_filename = f"{dataset_name}_{instance_name}_convergence_curves.png"
    convergence_path = os.path.join(convergence_dir, convergence_filename)
    plot_convergence_curves(convergence_data, save_path=convergence_path, dataset_name=dataset_name, instance_name=instance_name)
