...
def ga_new(parameters: dict, init_method: str = "random", return_convergence: bool = False):
    """
    遗传算法主函数
    :param parameters: 问题参数
    :param init_method: 初始化方法
    :param return_convergence: 是否返回收敛曲线
    :return: 最优makespan和收敛曲线（如果return_convergence为True）
    """
    # i = 1
    ...
    elif init_method == "FIFO_SPT":
        # FIFO + SPT 初始化方法
        population = encoding.initializePopulation_FIFO_SPT(parameters)
    elif init_method == "FIFO_EET":
        # FIFO + EET 初始化方法
        population = encoding.initializePopulation_FIFO_EET(parameters)
    elif init_method == "MOPNR_SPT":
        # MOPNR + SPT 初始化方法
        population = encoding.initializePopulation_MOPNR_SPT(parameters)
    elif init_method == "MOPNR_EET":
        # MOPNR + EET 初始化方法
        population = encoding.initializePopulation_MOPNR_EET(parameters)
    elif init_method == "LWKR_SPT":
        # LWKR + SPT 初始化方法
        population = encoding.initializePopulation_LWKR_SPT(parameters)
    elif init_method == "LWKR_EET":
        # LWKR + EET 初始化方法
        population = encoding.initializePopulation_LWKR_EET(parameters)
    elif init_method == "MWKR_SPT":
        # MWKR + SPT 初始化方法
        population = encoding.initializePopulation_MWKR_SPT(parameters)
    elif init_method == "MWKR_EET":
        # MWKR + EET 初始化方法
        population = encoding.initializePopulation_MWKR_EET(parameters)
    else:
        population = []
    # print("初始化种群：")
    # print(population)
    ...
    # 3.根据目标评估种群中的每一个个体。基于种群迭代（config.maxGen=100）
    while not termination.shouldTerminate(population, gen):
        ...
        best_makespan = max(task[1] for machine, tasks in gantt_data.items() for task in tasks)
    ...
    if return_convergence:
        return max_end_time, convergence_curve
    return max_end_time