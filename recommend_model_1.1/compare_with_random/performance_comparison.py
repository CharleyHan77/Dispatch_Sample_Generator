...
def load_test_results(result_dir, timestamp):
    ...
    recommended_files = list(Path(result_dir).glob(f"recommended_strategy_results_{timestamp}*.json"))
    ...
    if not recommended_files:
        # 如果没有找到指定时间戳的文件，查找最新的
        recommended_files = list(Path(result_dir).glob("recommended_strategy_results_*.json"))
...
def create_comparison_table(random_results, recommended_results, output_path):
    ...
    recommended_strategy = recommended_results['recommendation_info']['recommended_strategy']
    
    # 计算改进指标
    improvements = calculate_improvement_metrics(random_stats, recommended_stats)
    
    # 获取四维度评分
    random_scores = random_results.get('four_dimension_scores', {})
    recommended_scores = recommended_results.get('four_dimension_scores', {})
    ...
    comparison_data = {
        ...
        '随机初始化': [
            ...
            f"{random_scores.get('makespan_score', 0):.4f}",
            f"{random_scores.get('convergence_speed_score', 0):.4f}",
            f"{random_scores.get('stability_score', 0):.4f}",
            f"{random_scores.get('convergence_stability_score', 0):.4f}",
            "-",
            "-",
            "-",
            "-",
        ...
        f'推荐策略({recommended_strategy})': [
            ...
            f"{recommended_scores.get('makespan_score', 0):.4f}",
            f"{recommended_scores.get('convergence_speed_score', 0):.4f}",
            f"{recommended_scores.get('stability_score', 0):.4f}",
            f"{recommended_scores.get('convergence_stability_score', 0):.4f}",
...
def create_comparison_charts(random_results, recommended_results, output_dir):
    ...
    # 四维度评分对比
    score_names = ['Makespan评分', '收敛速度评分', '稳定性评分', '收敛稳定性评分']
    random_score_values = [
        random_scores.get('makespan_score', 0),
        random_scores.get('convergence_speed_score', 0),
        random_scores.get('stability_score', 0),
        random_scores.get('convergence_stability_score', 0)
    ]
    recommended_score_values = [
        recommended_scores.get('makespan_score', 0),
        recommended_scores.get('convergence_speed_score', 0),
        recommended_scores.get('stability_score', 0),
        recommended_scores.get('convergence_stability_score', 0)
    ]
    ...
    ax.plot(angles, values, 'o-', linewidth=2, color='#4ECDC4', label='改进率')
    ...
    for i, (angle, value) in enumerate(zip(angles[:-1], values[:-1])):
        ax.text(angle, value + max(values) * 0.05 if max(values) > 0 else value + 0.5, f'{value:.1f}%', 
...
## 测试概况
- **测试数据**: {random_results['fjs_file']}
- **推荐策略**: {recommended_strategy}
- **测试次数**: {random_results['execution_times']} 次
- **最大迭代数**: {random_results['max_iterations']}
- **测试时间**: {timestamp}

## 四维度性能指标对比

### 1. Makespan评分
- 随机初始化: {random_scores.get('makespan_score', 0):.4f}
- 推荐策略: {recommended_scores.get('makespan_score', 0):.4f}
- 改进率: {improvements['makespan_improvement']:.2f}%

### 2. 收敛速度评分
- 随机初始化: {random_scores.get('convergence_speed_score', 0):.4f}
- 推荐策略: {recommended_scores.get('convergence_speed_score', 0):.4f}
- 改进率: {improvements['convergence_improvement']:.2f}%

### 3. 稳定性评分
- 随机初始化: {random_scores.get('stability_score', 0):.4f}
- 推荐策略: {recommended_scores.get('stability_score', 0):.4f}
- 改进率: {improvements['stability_improvement']:.2f}%

### 4. 收敛稳定性评分
- 随机初始化: {random_scores.get('convergence_stability_score', 0):.4f}
- 推荐策略: {recommended_scores.get('convergence_stability_score', 0):.4f}
