...
class InitializationStrategyRecommender:
    ...
    def calculate_similarity(self, new_data_normalized, historical_fjs_path, max_basic_distance, max_processing_distance, new_data_features=None):
        ...
        # 5. 计算综合加权相似度（调整权重以增加差异化，统一所有初始化方法）
        weighted_similarity = (
            0.45 * basic_similarity +          # 大幅增加基础特征权重（作业数、机器数等）
            0.4 * processing_similarity +      # 增加加工时间特征权重（处理时间分布）
            0.1 * kde_similarity +             # 大幅降低KDE相似度权重
            0.05 * disjunctive_similarity      # 大幅降低析取图权重
        )
    ...
    def calculate_detailed_feature_similarity(self, new_data_normalized, historical_fjs_path, max_basic_distance, max_processing_distance, new_data_features=None):
        """
        计算详细的特征相似度，细化到每个具体指标
        ...
        # 基础特征权重分配（调整以增加差异化）
        basic_weights = {
            'num_jobs': 0.4,                    # 大幅增加作业数权重
            'num_machines': 0.35,               # 增加机器数权重
            'total_operations': 0.15,           # 降低总操作数权重
            'avg_available_machines': 0.05,     # 大幅降低平均可用机器数权重
            'std_available_machines': 0.05      # 大幅降低标准差权重
        }
        ...
        # 加工时间特征权重分配（调整以增加差异化）
        processing_weights = {
            'processing_time_mean': 0.45,       # 大幅增加平均加工时间权重
            'processing_time_std': 0.3,         # 增加标准差权重
            'processing_time_min': 0.1,         # 降低最小值权重
            'processing_time_max': 0.1,         # 降低最大值权重
            'machine_time_variance': 0.05       # 大幅降低机器时间方差权重
        }
        
        processing_weighted_similarity = sum(processing_weights[k] * processing_similarities[k] for k in processing_weights)
        
        # 四维特征综合相似度（调整权重以增加差异化，统一所有初始化方法）
        overall_similarity = (
            0.45 * basic_weighted_similarity +      # 大幅增加基础特征权重（作业数、机器数等）
            0.4 * processing_weighted_similarity +  # 增加加工时间特征权重（处理时间分布）
            0.1 * kde_similarity +                  # 大幅降低KDE相似度权重
            0.05 * disjunctive_similarity           # 大幅降低析取图权重
        )
