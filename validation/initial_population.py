...
from validation.FJSP_set import FJSP, calculate_makespan
...
if __name__ == '__main__':
    ...
    fjsp.parse_fjsp_data(parsed_data)
    print(fjsp.machines, fjsp.jobs)
    ...
    for quality in results.keys():
        for i in range(runs):
            ...
            print("*"* 40)
            print(f"第{i + 1}次执行的初始种群：{quality}")
            print(population)
            print("*" * 40)
            #print(population)
            #best_schedule = genetic_algorithm.genetic_algorithm(fjsp, population, generations)
            best_schedule = ga_new(population, generations)
            print(f"GA返回的最优序列：{best_schedule}")
    ...
    # for quality, metrics in results.items():
    #     mean_curve = np.mean(metrics["curves"], axis=0)
    #     plt.plot(mean_curve, label=quality)
    # plt.xlabel("迭代数")
    # plt.ylabel("Makespan")
    # plt.title("收敛曲线图")
    # plt.legend()
    ...
    # # 绘制收敛时间箱线图
    # plt.figure(figsize=(10, 6))
    # plt.boxplot([results[quality]["times"] for quality in results.keys()], labels=results.keys())
    ...
    # # 绘制早期收敛箱线图
    # plt.figure(figsize=(10, 6))
    # plt.boxplot([results[quality]["early_convergence"] for quality in results.keys()], labels=results.keys())
    # plt.xlabel("Initialization Strategy")
    # plt.ylabel("Early Convergence Iterations")
    # plt.title("Early Convergence Distribution")
    # plt.show()